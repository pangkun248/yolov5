...
def non_max_suppression(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, multi_label=False,
    ...
    # nms的相关配置
    min_wh, max_wh = 2, 7680  # box的最小与最大宽高(px)
    ...
    for xi, x in enumerate(prediction):  # 图片在batch_size中索引, 图片检测结果
        ...
        # Cat apriori labels if autolabelling  如果开启这个参数 则把pred_box与gt_box cat到一起进行NMS 如字面意思
...
def increment_path(path, exist_ok=False, sep='', mkdir=False):
    ...
    if path.exists() and not exist_ok:  # exist_ok=True时 代表不增加exp后面的数字,直接原地修改
        path, suffix = (path.with_suffix(''), path.suffix) if path.is_file() else (path, '')  # ''无后缀路径  路径后缀
