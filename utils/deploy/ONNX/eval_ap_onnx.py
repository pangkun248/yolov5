...
def calc_ap(onnx_path, val_path, anno_json, cls_list):
    ...
    for img_path in tqdm(img_list, desc=s):
        ...
        # trt run
        outputs = session.run(None, {session.get_inputs()[0].name: img})[0]  # Inference  bs=1 -> [0]
    ...
    pred_json = str(f"onnx_predictions.json")  # predictions json
...
if __name__ == "__main__":
    ...
    calc_ap(onnx_file, val_txt, val_json, coco_list)
