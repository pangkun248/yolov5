...
def calc_ap(xml_path, val_path, anno_json, cls_list):
    ie = IECore()
    net = ie.read_network(model=xml_path, weights=xml_path.replace('xml', 'bin'))
    # net = ie.read_network(xml_path.replace('xml', 'onnx'))
    ...
    for img_path in tqdm(img_list, desc=s):
        ...
        # trt run
        outputs = exec_net.infer(inputs={input_name: img})['output']  # Inference
    ...
    pred_json = str(f"trt_predictions.json")  # predictions json
...
if __name__ == "__main__":
    xml_path = "/home/cmv/res_new/yolov5s.xml"
    ...
    calc_ap(xml_path, val_txt, val_json, coco_list)
