...
def train(hyp,  # path/to/hyp.yaml or hyp dictionary
    ...
    w.mkdir(parents=True, exist_ok=True)  # 新建权重文件夹包括父级目录
    ...
    with torch_distributed_zero_first(RANK):
        data_dict = data_dict or check_dataset(data)  # check if None 训练集参数
    ...
    freeze = [f'model.{x}.' for x in range(freeze)]  # 冻结的参数名称 (全部或部分)
    ...
    optimizer.add_param_group({'params': g2})  # add g2 (biases) 这里不明白为什么要把g2单独放在一个group.明明和g0一样的优化配置
    ...
    imgsz = check_img_size(opt.imgsz, gs, floor=gs * 2)  # 验证img_size是否是gs的整数倍
...
def main(opt):
    ...
    if RANK in [-1, 0]:  # 非DDP 或者 当前机器为训练主机时(RANK=0)
        ...
        check_requirements(requirements=FILE.parent / 'requirements.txt', exclude=['thop'])  # 检测相应的py版本及requirements.txt中依赖的库,如果缺少则安装,但exclude除外
